...
## 1. 核心职责
- 驱动整个做市循环的业务流程。
- 从服务层收集数据，调用策略模块进行计算，并执行最终的交易决策。
...
1.  **数据收集**: 
    - 从 `MarketDataService` 获取 `midPrice` (s) 和 `volatility` (σ)。
    - 从 `AccountService` 获取 `normalizedInventory` (q)。
    - 从 `Config` 获取策略参数 `riskAversion` (γ), `baseSpread`, `orderAmount` 等。

2.  **策略计算**: 
    - 调用 `Strategy.calculateQuotes()` 并传入收集到的数据，获取理论上的 `theoreticalBid` 和 `theoreticalAsk`。
...
4.  **风险检查 (Pre-trade Checks)**:
    - **库存检查**: 检查如果新订单成交，总持仓是否会超过 `MAX_INVENTORY`。 `(currentPosition + openOrders + newOrder) <= maxPosition`。
...
## 5. 与其他模块的交互
- **依赖**: `Strategy`, `ExchangeService`, `MarketDataService`, `AccountService`, `Config`, `Logger`, `Utils`。
- **被依赖**: `main.js` 模块会创建和启动 `Trader` 实例。
- **协调者角色**: `Trader` 是所有模块的“协调者”。它不直接实现任何功能，而是通过调用其他模块的服务来完成复杂的业务逻辑。
