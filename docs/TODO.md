...
### 10. Main.js (`src/main.js`) ✅ 已完成

- [x] 实现程序启动流程：加载配置 -> 初始化日志 -> 初始化所有服务。
- [x] 启动 `WebSocketManager` 连接并进行初始账户同步。
- [x] 遍历配置文件中的交易对，为每个交易对创建并启动一个 `Trader` 实例。
- [x] 实现全局异常处理 (`uncaughtException`, `unhandledRejection`)。
- [x] 实现优雅退出逻辑 (`SIGINT`, `SIGTERM`)，确保退出前取消所有挂单。
...
#### 12.4 系统集成测试
- [x] **基础架构测试** - 已完成前两阶段的全面测试：
  - [x] 配置管理测试（Config类、环境变量加载）
  - [x] 交易所服务测试（ExchangeService、REST API调用）
  - [x] WebSocket管理测试（连接、数据订阅、事件处理）
  - [x] 数据获取测试（实时价格、账户信息、市场数据）
  - [x] 模拟盘连接验证（Bitget模拟盘成功对接）
- [x] **Bitget双向持仓模式修复** - 已完成：
  - [x] 修复40774错误（缺少positionSide参数）
  - [x] 强制设置双向持仓模式兼容所有TRADE_SIDE配置
  - [x] 增强平仓逻辑，智能判断并添加reduceOnly参数
  - [x] 创建专门的Bitget双向持仓模式测试用例
  - [x] 验证修改后代码的正确性和稳定性
